<script lang="ts">
	...
	let fileName = $state('');
	// $state.raw: sheets hold up to thousands of rows and are only reassigned
	let workbook: Workbook | null = $state.raw(null);
	...
	onMount(() => {
		...
		(async () => {
			...
			if (received.status === 'ok') {
				await loadBytes(received.bytes, 'shared.xlsx');
			} else {
				loading = false;
				error =
					received.status === 'cancelled'
						? 'Password required to view this workbook. Reload the page to try again.'
